@extends('layouts.header')

@include('partials.adminNav')

@section('content')

<div style="display: flex;">
	<div style="flex-grow: 1">
		<h1 align="center">Cuisines</h1>
<form method="POST" action="{{route('postAddFoodType')}}" enctype="multipart/form-data">
	@csrf
	<input type="text" class="form-control" placeholder="Type Name" name="name" required>
	<div class="form-group">
        <label for="content">Photo</label>
        <input type="file" class="form-control" id="photo_path" name="photo_path">
    </div>
	<button type="submit" class="btn btn-success">Add</button>
</form>

<table>
	<thead>
        <tr>
            <th style="padding: 10px;">Name</th>
            <th style="padding: 10px;">Photo</th>
            <th style="padding: 10px;">Delete</th>
        </tr>
    </thead>
    <tbody>
    @foreach($types as $type)
        <tr>
            <td>{{$type->name}}</td>
            <td><img style="width: 50px;" src="{{URL::to($type->photo_path)}}"></td>
            <td><a href="{{ route('deleteGetFoodType', ['id'=> $type->id ]) }}">Delete</a></td>
        </tr>
    @endforeach
	</tbody>
</table>
	</div>
	<div style="flex-grow: 1">
		<h1 align="center">Types</h1>

<form method="POST" action="{{route('postAddFoodCuisine')}}" enctype="multipart/form-data">
	@csrf
	<input type="text" class="form-control" placeholder="Cuisine Name" name="name" required>
	<div class="form-group">
        <label for="content">Photo</label>
        <input type="file" class="form-control" id="photo_path" name="photo_path">
    </div>
	<button type="submit" class="btn btn-success">Add</button>
</form>

<table>
	<thead>
        <tr>
            <th style="padding: 10px;">Name</th>
            <th style="padding: 10px;">Photo</th>
            <th style="padding: 10px;">Delete</th>
        </tr>
    </thead>
    <tbody>
    @foreach($cuisines as $cuisine)
        <tr>
            <td>{{$cuisine->name}}</td>
            <td><img style="width: 50px;" src="{{URL::to($cuisine->photo_path)}}"></td>
            <td><a href="{{ route('deleteGetFoodCuisine', ['id'=> $cuisine->id ]) }}">Delete</a></td>
        </tr>
    @endforeach
	</tbody>
</table>
	</div>
</div>
<h1 align="center">Foods</h1>

<a href="{{route('addGetFood')}}" class="btn btn-success">Add</a>

<table>
        <thead>
            <tr>
                <th style="padding: 10px;">Name</th>
                <th style="padding: 10px;">Description</th>
                <th style="padding: 10px;">Type</th>
                <th style="padding: 10px;">Cuisine</th>
                <th style="padding: 10px;">Price</th>
                <th style="padding: 10px;">Photo</th>
                <th style="padding: 10px;">Edit</th>
                <th style="padding: 10px;">Delete</th>
            </tr>
        </thead>
        <tbody>
        @foreach($foods as $food)

            <tr>
                <td>{{$food->name}}</td>
                <td>{{$food->description}}</td>
                <td>{{$food->type->name}}</td>
                <td>{{$food->cuisine->name}}</td>
                <td>{{$food->price}}</td>
                <td><img style="width: 50px;" src="{{URL::to($food->photo_path)}}"></td>
                <td><a href="{{ route('editGetFood', ['id'=> $food->id ]) }}">Edit</a></td>
                <td><a href="{{ route('deletePostFood', ['id'=> $food->id ]) }}">Delete</a></td>
            </tr>

        @endforeach

    </tbody>

</table>

@endsection